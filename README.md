...
> 说明：社区参考实现（legadoM-Ink）给出的横屏方向与 iReader Air3 Pro 实测**相反**，本插件采用实测值
> （竖屏两个方向原样正确，横屏两个方向已对调）。若你的机型出现镜像，改
> `lib/epdc.lua` 里 `M.config.directions_forward` / `directions_backward` 的下标 1 与 3 即可。
...
## 更新日志
- **0.7.0** —— 修复横屏（左/右横屏）水波纹方向与实际翻页方向相反的问题：方向码表中两个横屏表项对调（竖屏不受影响）。
- **0.6.0** —— 「启用」改为真正的开关（去掉置灰、显式保存并立刻落盘，开启时顺带清除旧的崩溃禁用标记）；
  翻页速度选择后立即 `flush()`，重启不再回到默认；移除「其他」与「前光」菜单项（前光转为默认关闭、仅保留代码）。
